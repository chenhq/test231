...
def tag_wave_direction(ohlcv, first_count_threshold, return_per_count_threshold,
                       withdraw_threshold, operation='search', mode='relative', std_window=30):
    ...
    while i < len(ohlcv):
        ...
        if ohlcv.iloc[i]['close'] > ohlcv.iloc[i]['open']:
            ...
            direction = 1
            returns = (ohlcv.iloc[i]['close'] / ohlcv.iloc[i]['open']) - 1
            if returns < first_count_threshold * ohlcv.iloc[i]['std']:
                i += 1
                continue
            ...
            direction = -1
            returns = 1 - (ohlcv.iloc[i]['close'] / ohlcv.iloc[i]['open'])
            if returns < first_count_threshold * ohlcv.iloc[i]['std']:
                i += 1
                continue
        else:
